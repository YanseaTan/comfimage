...
Comfimage 是一个现代化的 Web 界面，用于通过 ComfyUI 生成 AI 图像。采用前后端分离架构，前端使用原生 HTML/CSS/JavaScript，后端使用 Flask 框架。支持用户登录、图像生成、历史记录管理等功能。

## 功能特性

- 🔐 **用户管理系统**：支持多用户登录、密码修改
- 🎨 **AI 图像生成**：支持自定义提示词生成图像
- 📐 **参数调节**：可调节图像宽度、高度（512-2048px）
- 🎲 **种子控制**：支持种子设置以复现生成结果
- 📊 **实时进度**：生成过程中显示实时进度条
- 🖼️ **图像预览**：支持全屏查看和下载图片
- 📚 **历史记录**：保存生成历史，支持参数回填
...
5. **准备工作流文件**：
   - 确保 `config/z_image_turbo.json` 文件存在
   - 该文件包含 ComfyUI 的工作流配置
...
- **z_image_turbo.json**: ComfyUI 工作流模板文件
- 支持自定义节点ID映射和参数注入
- 可根据需要修改生成参数和模型配置
